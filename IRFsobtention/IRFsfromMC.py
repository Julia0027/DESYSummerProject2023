...
T_OBS = 5 * u.hour
arrayconfiguration='N.BL-4LSTs15MSTs-MSTN'
outputfilename="pyirfs_4LSTs15MSTs_5h.fits.gz"
#---------------TO CHANGE---------------
#
current_directory = os.path.dirname(os.path.abspath(__file__))
file_directory = os.path.abspath(os.path.join(current_directory, '../../'))
#gamma
prefile_gamma = 'gamma_onSource.'+ arrayconfiguration +'.fits.gz'
file_gamma=os.path.join(file_directory, prefile_gamma)
#proton
prefile_proton = 'proton_onSource.'+ arrayconfiguration +'.fits.gz'
file_proton=os.path.join(file_directory, prefile_proton)
#electron
prefile_electron = 'electron_onSource.'+ arrayconfiguration +'.fits.gz'
file_electron=os.path.join(file_directory, prefile_electron)
...
def main():
    ...
    mask_theta_cuts = gammas["gh_score"] >= INITIAL_GH_CUT #initial value, try changing the one in the plot
    theta_cuts_coarse = calculate_percentile_cut(
        ...
        fill_value=0.32 * u.deg,  #for the lowest energies maybe, in case there are no values
